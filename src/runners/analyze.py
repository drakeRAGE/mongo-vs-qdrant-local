...
def _line_by_engine(ax, frame: pd.DataFrame, y: str, xlabel: str, ylabel: str, title: str, hline=None) -> None:
    for engine, g in frame.groupby("engine"):
        g = g.sort_values("n")
        ax.plot(g["n"], g[y], marker="o", label=engine)
    ...
    if hline is not None:
        ax.axhline(hline, linestyle="--", linewidth=0.8)
...
def _plot_p95_vs_n(df: pd.DataFrame) -> Path | None:
    ...
    _line_by_engine(ax, med, "p95_ms", "N vectors", "p95 latency (ms)", "E1 iso-config: p95 vs N", 50)
...
def _plot_p99_vs_n(df: pd.DataFrame) -> Path | None:
    ...
    fig, ax = plt.subplots(figsize=(7, 4))
    _line_by_engine(ax, med, "p99_ms", "N vectors", "p99 latency (ms)", "E1 iso-config: p99 vs N", 100)
...
def _plot_pareto(df: pd.DataFrame) -> Path | None:
    ...
    for (engine, n), g in sub.groupby(["engine", "n"]):
        ...
        ax.plot(g["p95_ms"], g["recall_at_10"], marker="o", label=f"{engine} n={int(n)}")
    ax.set_xlabel("p95 latency (ms)")
    ax.set_ylabel("Recall@10")
    ax.set_title("Iso-recall sweep: Recall@10 vs p95")
    ax.axhline(0.95, linestyle="--", linewidth=0.8)
    ax.legend()
...
def _plot_iso_recall_ef(df: pd.DataFrame) -> Path | None:
    ...
    for ax, y, title in (
        ...
        for (engine, n), g in sub.groupby(["engine", "n"]):
            g = g.sort_values("ef")
            ax.plot(g["ef"], g[y], marker="o", label=f"{engine} n={int(n)}")
        ax.set_xlabel("ef / numCandidates")
        ax.set_ylabel(y.replace("_", " "))
        ax.set_title(title)
        ax.legend(fontsize=8)
    axes[1].axhline(0.95, linestyle="--", linewidth=0.8)
...
def _plot_rss(df: pd.DataFrame) -> Path | None:
    ...
    fig, ax = plt.subplots(figsize=(7, 4))
    _line_by_engine(ax, med, "rss_mb", "N vectors", "Container RSS (MB)", "E6: RSS vs N (E1 iso-config cells)")
...
def _plot_filter(df: pd.DataFrame) -> Path | None:
    ...
    for (engine, n), g in sub.groupby(["engine", "n"]):
        g = g.sort_values("sel")
        ax.plot(g["sel"], g["p95_ms"], marker="o", label=f"{engine} n={int(n)}")
    ax.set_xlabel("Selectivity (%)")
    ax.set_ylabel("p95 latency (ms)")
    ax.set_title("E3 filtered ANN, concurrency=1")
...
def _plot_concurrency_p95(df: pd.DataFrame) -> Path | None:
    ...
    for (engine, n), g in sub.groupby(["engine", "n"]):
        ...
        ax.plot(g["concurrency"], g["p95_ms"], marker="o", label=f"{engine} n={int(n)}")
...
def _plot_concurrency_qps(df: pd.DataFrame) -> Path | None:
    ...
    for (engine, n), g in sub.groupby(["engine", "n"]):
        g = g.sort_values("concurrency")
        ax.plot(g["concurrency"], g["qps"], marker="o", label=f"{engine} n={int(n)}")
    ax.set_xlabel("Concurrency")
    ax.set_ylabel("QPS")
...
def _plot_cold(df: pd.DataFrame) -> Path | None:
    ...
    for engine, g in e5.groupby("engine"):
        g = g.sort_values("n")
        ax.plot(g["n"], g["p95_ms"], marker="o", label=f"{engine} cold")
    ax.set_xscale("log")
    ax.set_xlabel("N vectors")
    ax.set_ylabel("p95 latency (ms)")
...
def write_results_md(df: pd.DataFrame, plots: dict[str, Path]) -> Path:
    ...
    if e1.empty:
        ...
        lines.append("")
        for key in ("e1_p95_vs_n", "e1_p99_vs_n", "e1_qps_vs_n", "e1_recall_vs_n", "e1_p95_ratio_vs_n"):
